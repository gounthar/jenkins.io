...
The Jenkins project provides Linux container images but also Windows container images.
This tutorial focuses on the Linux container images only, so ensure that your Docker for Windows installation is configured to run `Linux Containers` rather than `Windows Containers`.
