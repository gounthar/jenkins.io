...
To install Docker and Docker Compose on your operating system, check you have the correct prerequisites et the <<prerequisites,Prerequisites>> below) and then visit link:https://hub.docker.com/search?type=edition&offering=community[Docker Hub], and select the *Docker Community Edition* suitable for your operating system or cloud service.
