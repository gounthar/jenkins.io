...
The second method we propose (<<quickstart-tutorials-repository>>) is the easiest way to get Jenkins up and running.
However, you may want to have a better feel of what's happening under the hood. +
This is where the first method comes in. +
...
If you have already launched the Jenkins instance previously, you can stop it by running the command `docker compose --profile wizard down -v --remove-orphans`. +
