...
. Click on *Add*.
. In the *Key* field, paste the content of the `jenkins_agent_ed` file you can find in the `wizard_agent` thanks to the following command:

[source,bash]
----
docker compose exec wizard_agent cat /home/jenkins/.ssh/jenkins_agent_ed
----
