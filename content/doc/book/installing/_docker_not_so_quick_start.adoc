...
We'll need two terminals for this. One will be used to run the `docker compose` command, and the other will be used to check the status of the containers and get some information.
...
Once the containers are running successfully, open a second terminal, and navigate to the same directory.
Verify all is going well thanks to the `docker compose ps` command in the second terminal. +
The controller can now be accessed at http://localhost:8080.
