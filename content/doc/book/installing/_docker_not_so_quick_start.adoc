...
This container is already running, as it started with the `docker compose` command.
...
. Click on *Add*.
. Switch to the second terminal and enter this command:  `docker compose exec wizard_agent cat /home/jenkins/.ssh/jenkins_agent_ed`.
This will copy the content of the `jenkins_agent_ed` file in the `wizard_agent` container to the terminal.
. Copy it to your clipboard, beginning with `-----BEGIN OPENSSH PRIVATE KEY-----` and ending with `-----END OPENSSH PRIVATE KEY-----` and paste it in the *Key* field.
[.boxshadow]
...
Should you need to access the services logs, you can do so by running the following command in the second terminal:
[source,bash]
----
docker compose --profile wizard logs -f
----

This command will display the logs of all services running in the dockerized Jenkins controller and agent combination.
